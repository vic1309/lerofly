...
Although we can say that those two observations are general rules of Euler's Method one can ask itself: *"Does it work ever?"* or *"How do I know if my numerical solution is accurate once that usually we don't have the analytical solution?"* or given the prior example: *"If my step size goes to 0 so does the error?"*. These questions lead us to the next topic  
